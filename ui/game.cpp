...
bool Game::init()
{
    ...
    milkTex.load(":/assets/assets/Milk.png");
    eggTex.load(":/assets/assets/Brown_Egg");
...
void Game::keyPressEvent(QKeyEvent *event)
{
    ...
    if (key == Qt::Key_T) { advanceGrowthCycle(); }
...
void Game::saveGame(int saveSlot) {
    SaveManager::getInstance().saveGame(saveSlot, player.get(), &tiles, dayNumber);
}

void Game::loadGame(int saveSlot) {
    if (SaveManager::getInstance().saveExists(saveSlot)) {
        SaveManager::getInstance().loadGame(saveSlot, player.get(), &tiles, dayNumber);
    }
}
...
const QPixmap* Game::getItemSprite(ItemTypeEnum itemType) const {
    ...
    case ItemTypeEnum::MILK: return &milkTex;
    case ItemTypeEnum::EGG: return &eggTex;
...
void Game::renderTileIndicator(QPainter& painter)
{
    ...
    QColor overlayColor(255, 255, 0, 50);
